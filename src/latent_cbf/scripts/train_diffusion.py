...
@hydra.main(config_path=os.path.join(base_path, '../diffusion4robotics/experiments'), config_name="finetune.yaml")
def bc_finetune(cfg: DictConfig):
    try:
        ...
        trainer = hydra.utils.instantiate(cfg.trainer, model=agent, device_id=1)
        ...
        for itr in (pbar := tqdm.tqdm(range(cfg.max_iterations), postfix=dict(Loss=None))):
            ...
            if misc.GLOBAL_STEP >= cfg.max_iterations:
                ...
                trainer.save_checkpoint(cfg.checkpoint_path, misc.GLOBAL_STEP)
                save_path = cfg.checkpoint_path.split('.')[0] + str(misc.GLOBAL_STEP) + '.ckpt'
