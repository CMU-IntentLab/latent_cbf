...
class avoid_DDPGPolicy_annealing_acreg(BasePolicy):
    ...
    def learn(self, batch: Batch, **kwargs: Any) -> Dict[str, float]:
        ...
        if not self.warmup:
            for _ in range(self.actor_gradient_steps):
                ...
                act_raw = ((act + 1) * 0.5) * (self.max_ac - self.min_ac) + self.min_ac
                # Penalize large unnormalized actions
                action_reg_loss = (act_raw[:, :6] ** 2).sum(dim=1).mean()
                action_mag = torch.norm(act_raw[:, :6], dim=-1, keepdim=True)
                safety_loss = -self.critic(batch.obs, act).mean()
                actor_loss = safety_loss + 1e-2 * action_reg_loss # encourage smaller actions
