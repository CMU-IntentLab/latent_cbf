...
class Logger:
    def __init__(self, logdir, step):
        ...
        # Initialize WandB
        wandb.init(project="latent_cbf", config={"logdir": str(logdir)}, name=name)
...
class DebugLogger:
    """
    Debug logger that has the same interface as Logger but doesn't call wandb.
    Useful for debugging or when you don't want to log to wandb.
    """
    def __init__(self, logdir, step):
        self._logdir = logdir
        self._last_step = None
        ...
        print(f"DebugLogger initialized for logdir: {logdir}")
    ...
    def config(self, config_dict):
        ...
        # Just print config instead of logging to wandb
    ...
    def write(self, fps=False, step=False, fps_namespace="", print_cli=True):
        ...
        # Optional: Save scalars to a simple text file for debugging
        if scalars:
            log_file = os.path.join(self._logdir, "debug_scalars.txt")
            with open(log_file, "a") as f:
                f.write(f"Step {step}: " + " / ".join(f"{k} {v:.6f}" for k, v in scalars) + "\n")
        
        # Just print info about images/videos instead of logging to wandb
        if self._images:
            print(f"Debug: {len(self._images)} images logged (not saved)")
        if self._videos:
            print(f"Debug: {len(self._videos)} videos logged (not saved)")
    ...
    def offline_scalar(self, name, value, step):
        # Just print instead of logging to wandb
        print(f"Offline scalar [{step}]: {name} = {value}")

    def offline_video(self, name, value, step):
        # Just print info instead of logging to wandb
        print(f"Offline video [{step}]: {name} shape = {value.shape}")
...
def fill_offline_dataset(config, cache, eval_cache):
    ...
    with h5py.File(dataset_path, 'r') as f:
        demos = f
        print(demos.keys())
        trajectories = demos['trajectories']
        ...
        for i in tqdm(
            ...
            for t in range(T):
                ...
                for obs_key in pixel_keys:
                    transition[obs_key] = observations[t]

                if len(state_keys) != 0:
                    theta_t = states[t, -1]
                    curr_obs_state_vec = [theta_t]
                    transition["state"] = curr_obs_state_vec
                    
                transition["privileged_state"] = states[t]
                transition["obs_state"] = [np.cos(transition["state"][0]), np.sin(transition["state"][0])]
                ...
                if t == 0:
                    ...
                    transition["is_first"] = np.array(False, dtype=np.bool_)
